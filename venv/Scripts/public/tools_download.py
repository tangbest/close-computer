...
class CDownloaderBase(QThread):
    ...
    def run(self):
        ...
        while self.m_oEventRunning.isSet(): # 如果被设置为了true就继续，false就终止了
            ...
            with contextlib.closing(requests.get(self.m_sUrl, headers = dctHeaders, stream=True)) as oRequest:
                print("状态码: ", oRequest.status_code)
                if oRequest.status_code != 200:
                    print("状态码有误,下载失败")
                    return
                #print("内容", oRequest.text)
                #print("内容", oRequest.json())
                ...
                try:
                    ...
                    with open(self.m_sSavePath, "wb") as oFile:
                        ...
                        for data in oRequest.iter_content(chunk_size=1024):
